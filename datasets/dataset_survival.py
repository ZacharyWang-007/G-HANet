...
class Generic_WSI_Survival_Dataset(Dataset):
    ...
    def return_splits(self, from_id: bool=True, csv_path: str=None):
        if from_id:
            ...
            # to normalize the genomic data
            # temp = np.concatenate((train_split.genomic_features, val_split.genomic_features), axis=0)
            # max, min = temp.max(axis=0), temp.min(axis=0)
            
            # train_split.genomic_features_norm = (train_split.genomic_features - min) / (max - min)
            # val_split.genomic_features_norm = (val_split.genomic_features - min) / (max - min)
...
class Generic_MIL_Survival_Dataset(Generic_WSI_Survival_Dataset):
    def __init__(self, data_dir, max=None, min=None, mode: str='omic', **kwargs):
        ...
        if not self.use_h5:
            if self.data_dir:
                if self.mode == 'path':
                    ...
                    # omic1_norm = torch.tensor(self.genomic_features_norm[self.omic_names[0]].iloc[idx])
                    # omic2_norm = torch.tensor(self.genomic_features_norm[self.omic_names[1]].iloc[idx])
                    # omic3_norm = torch.tensor(self.genomic_features_norm[self.omic_names[2]].iloc[idx])
                    # omic4_norm = torch.tensor(self.genomic_features_norm[self.omic_names[3]].iloc[idx])
                    # omic5_norm = torch.tensor(self.genomic_features_norm[self.omic_names[4]].iloc[idx])
                    # omic6_norm = torch.tensor(self.genomic_features_norm[self.omic_names[5]].iloc[idx])

                    # omic1 = torch.cat((omic1, omic1_norm), dim=0)
                    # omic2 = torch.cat((omic2, omic2_norm), dim=0)
                    # omic3 = torch.cat((omic3, omic3_norm), dim=0)
                    # omic4 = torch.cat((omic4, omic4_norm), dim=0)
                    # omic5 = torch.cat((omic5, omic5_norm), dim=0)
                    # omic6 = torch.cat((omic6, omic6_norm), dim=0)
...
class Generic_Split(Generic_MIL_Survival_Dataset):
    def __init__(self, slide_data, metadata, mode, signatures=None, data_dir=None, label_col=None, patient_dict=None, num_classes=2):
        ...
        # breakpoint()
        # max, min = self.genomic_features.max(axis=0), self.genomic_features.max(axis=0)
        # self.genomic_features = (self.genomic_features - min) / (max - min)
        # breakpoint()
        ### <--
