...
def main(args):
	...
	for i in folds:
		...
		if 'omic' in args.mode or args.mode == 'cluster' or args.mode == 'graph' or args.mode == 'pyramid':
			...
			args.omic_sizes = [94,334,521,468,1495,479] #train_dataset.omic_sizes
...
parser.add_argument('--data_root_dir',   type=str, default='/media/zhikangwang/MedicalData/TCGA/Bladder', help='Data directory to WSI features (extracted via CLAM')
...
if __name__ == "__main__":
	start = timer()
	sys.stdout = Logger(os.path.join('./log', str(len(os.listdir('./log'))) + '.txt'))
	results = main(args)
	end = timer()
	print("finished!")
	print("end script")
